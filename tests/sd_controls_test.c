...
static bool pins[30];static uint32_t now;static unsigned volume_events,enable_events;static bool last_up,last_down;
...
void cec_arc_set_enabled(bool enabled){assert(enabled);++enable_events;}
...
int main(void){sd_controls_init();assert(pins[9]&&pins[12]&&pins[20]&&pins[26]);
 ...
 key(10,true);key(10,false);assert(sd_controls_playing()&&enable_events==1);key(13,true);key(13,false);assert(sd_controls_next_generation()==n+1);
 ...
 puts("PASS: B1 play/stop LEDs, B2 next and playback-only B2/B3/B4 controls");}
